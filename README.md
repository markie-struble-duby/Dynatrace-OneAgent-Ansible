...
| `dynatrace_oneagent_install_args` | `APP_LOG_CONTENT_ACCESS=1 INFRA_ONLY=0` | Dynatrace OneAgent install parameters defined as a list of items. Ignored if dynatrace_use_oneagentctl = yes
| `dynatrace_oneagent_install_args_new` | `set-app-log-content-access=true set-infra-only=false` | Dynatrace OneAgent install parameters defined as a list of items, used for --set-param method. Ignored if dynatrace_use_oneagentctl = no
